...
pub fn sign_es256_jws(secret_key: &[u8], payload_text: &str) -> Result<String, JwsEs256Error> {
    ...
    let raw64 = sign_p256_jose_prehash(secret_key, signing_input.signing_input.as_bytes())?;

    Ok(encode_compact_jws(signing_input, raw64.as_ref()))
}
...
pub fn verify_es256_jws(jws: &str, public_key_sec1: &[u8]) -> Result<(), JwsEs256Error> {
    ...
    verify_p256_jose_prehash(&raw, signing_input.as_bytes(), public_key_sec1)
}
