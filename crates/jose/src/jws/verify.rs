...
pub(crate) fn decode_signature<E>(signature: &str, bad_base64: E) -> Result<Zeroizing<Vec<u8>>, E> {
    base64url_bytes_to_bytes(signature.as_bytes())
        .map(Zeroizing::new)
        .map_err(|_| bad_base64)
}

pub(crate) fn decode_payload<E>(payload: &str, bad_base64: E) -> Result<Zeroizing<Vec<u8>>, E> {
    base64url_bytes_to_bytes(payload.as_bytes())
        .map(Zeroizing::new)
        .map_err(|_| bad_base64)
}
