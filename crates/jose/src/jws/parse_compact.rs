...
pub(crate) fn build_sig_structure<E>(
    ...
    // The encoded payload can be reversed into plaintext claims or other PII.
    // Make zeroization an invariant of the shared constructor so signing and
    // verification call sites cannot accidentally choose a weaker owner.
    let mut structure = Zeroizing::new(Vec::with_capacity(len));
