...
fn derive_ecdh_es_content_encryption_key_for_len<const N: usize>(
    ...
    let derived_bytes = derived.into_zeroizing();
    Ok(Zeroizing::new(derived_bytes.to_vec()))
}

fn decode_party_info(
    value: Option<&str>,
) -> Result<reallyme_crypto::concat_kdf::JwaPartyInfo, JweError> {
    // Party info is commonly public metadata, but applications can place
    // correlating or privacy-sensitive bytes in it. Keep the decoded
    // intermediate under the same zeroization policy as the parsed header.
