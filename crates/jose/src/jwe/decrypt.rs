...
pub fn decrypt_compact_jwe_bytes(
    ...
    crate::operation_contract::jwe::decrypt_jwe(compact_jwe, policy, key_resolver)
}

pub(crate) fn decrypt_compact_jwe_bytes_core(
    compact_jwe: &str,
    policy: &CompactJwePolicy<'_>,
    key_resolver: &dyn JweContentEncryptionKeyResolver,
) -> Result<Zeroizing<Vec<u8>>, JweError> {
    ...
    let encrypted_key = Zeroizing::new(
    ...
    let iv = Zeroizing::new(base64url_to_bytes(parts.iv).map_err(|_| JweError::InvalidEncoding)?);
    let ciphertext = Zeroizing::new(
        base64url_to_bytes(parts.ciphertext).map_err(|_| JweError::InvalidEncoding)?,
    );
    let tag = Zeroizing::new(base64url_to_bytes(parts.tag).map_err(|_| JweError::InvalidEncoding)?);
...
fn join_ciphertext_and_tag(
    ...
    reallyme_crypto::aes::CiphertextWithTag::from_vec(core::mem::take(&mut ciphertext))
