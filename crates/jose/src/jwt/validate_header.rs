            ...
            Some(typ) => {
                if options.accepted_typ_values.is_empty()
                    || !options
                        .accepted_typ_values
                        .iter()
                        .any(|accepted| jwt_typ_matches(typ, accepted))
                {
...
pub struct JwtHeaderValidationOptions<'a> {
    ...
    /// Accepted `typ` media types. Comparison is ASCII case-insensitive and
    /// treats the optional `application/` prefix as equivalent.
