...
impl Drop for UnsignedJwtHeader {
    fn drop(&mut self) {
        self.alg.zeroize();
        self.typ.zeroize();
    }
...
pub fn encode_unsigned_jwt<Claims: Serialize>(claims: &Claims) -> Result<String, JwtError> {
    ...
    crate::operation_contract::jwt::encode_unsigned_jwt(&claims_json)
}
...
pub fn decode_unsigned_jwt<Claims: DeserializeOwned>(jwt: &str) -> Result<Claims, JwtError> {
    let payload_bytes = crate::operation_contract::jwt::decode_unsigned_jwt(jwt)?;
...
pub fn decode_unsigned_jwt_claims_json(jwt: &str) -> Result<Zeroizing<Vec<u8>>, JwtError> {
    crate::operation_contract::jwt::decode_unsigned_jwt(jwt)
}

pub(crate) fn decode_unsigned_jwt_claims_json_core(
    jwt: &str,
