...
/// Registered identity claim identifiers used in typed validation errors.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum JwtRegisteredClaim {
    /// Issuer claim (`iss`).
    Issuer,
    /// Subject claim (`sub`).
    Subject,
...
pub enum JwtError {
    ...
    /// The verifier supplied an invalid or absent current time.
    #[error("invalid JWT verification time")]
    ...
    /// A registered identity claim required by verifier policy was absent.
    #[error("missing required registered JWT claim: {0:?}")]
    MissingRequiredRegisteredClaim(JwtRegisteredClaim),

    /// A registered identity claim was malformed.
    #[error("invalid registered JWT claim value: {0:?}")]
    InvalidRegisteredClaimValue(JwtRegisteredClaim),

    /// The token audience does not include the intended recipient.
    #[error("JWT audience mismatch")]
    AudienceMismatch,

    /// The token issuer does not match verifier policy.
    #[error("JWT issuer mismatch")]
    IssuerMismatch,

    /// The token subject does not match verifier policy.
    #[error("JWT subject mismatch")]
    SubjectMismatch,

    /// The verifier supplied an unsafe or incomplete registered-claims policy.
    #[error("invalid JWT claims validation policy")]
    InvalidClaimsPolicy,
}
