...
pub fn encode_signed_jwt_with_header_options<C: Serialize>(
    ...
    crate::operation_contract::jwt::sign_jwt(&payload_json, jwk, private_key, header_options)
}
...
pub fn encode_signed_jwt_with_signer_and_header_options<C: Serialize>(
    ...
    let payload_json =
        Zeroizing::new(serde_json::to_vec(claims).map_err(|_| JwtError::Serialization)?);
    crate::operation_contract::jwt::sign_jwt_with_signer(&payload_json, jwk, signer, header_options)
}

pub(crate) fn encode_signed_jwt_claims_json_core(
    claims_json: &[u8],
    jwk: &Jwk,
    private_key: &[u8],
    header_options: &JwtHeaderEncodeOptions,
...
pub(crate) fn encode_signed_jwt_claims_json_with_signer_core(
    claims_json: &[u8],
    jwk: &Jwk,
    signer: &dyn Signer,
    header_options: &JwtHeaderEncodeOptions,
    ...
    let backend_signature = signer
        .sign(&signing_input.signing_input)
        .map_err(|_| JwtError::Crypto)?;
    let signature = encode_signature_for_jwt(crypto_alg, backend_signature)?;
    validate_signature_key_binding(crypto_alg, jwk, &signing_input.signing_input, &signature)?;
    encode_signed_compact_jwt(signing_input, &signature)
}
