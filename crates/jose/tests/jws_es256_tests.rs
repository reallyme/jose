...
#[test]
fn jws_es256_returns_empty_authenticated_payload() {
    let (public, private) = generate_keypair(Algorithm::P256).unwrap();
    let jws = sign_es256_jws(&private, "").unwrap();
...
#[test]
fn jws_es256_rejects_authenticated_invalid_payload_base64url() {
    let (public, private) = generate_keypair(Algorithm::P256).unwrap();
    ...
    let error = verify_es256_jws_and_decode_payload(&jws, &public).unwrap_err();

    assert_eq!(error, JwsEs256Error::BadPayloadBase64);
}
...
fn jws_es256_accepts_high_s_signature_as_valid_ecdsa() {
    ...
    let authenticated = verify_es256_jws_and_decode_payload(&jws, &public).unwrap();
    assert_eq!(authenticated.as_slice(), b"test");
}
...
fn jws_es256_rejects_all_zero_signature_scalars() {
    ...
    assert_eq!(err, JwsEs256Error::BadRawSignature);
}
