...
//! closed with the exact `JwtError` variant named in the checked-in fixture.
//! Fixture changes must remain explicit and independently reviewable.
...
fn signed_jwt_vectors_verify_or_fail_closed() {
    let suite: Value = serde_json::from_str(include_str!("../../../vectors/signed-jwt.json"))
        ...
        let result = match case.get("now_unix").and_then(Value::as_u64) {
            Some(now) => {
                let policy = match field(case, "temporal_policy") {
                    Some("strict") | None => JwtClaimsValidationPolicy::strict("did:me:verifier"),
                    Some(other) => panic!("{id}: unknown temporal_policy {other}"),
                };
                decode_verify_jwt_with_claims_validation::<Value>(
                    compact,
                    &jwk,
                    &public_key,
                    now,
...
#[cfg_attr(target_arch = "wasm32", wasm_bindgen_test::wasm_bindgen_test)]
#[cfg_attr(not(target_arch = "wasm32"), test)]
fn signed_jwt_rejects_expiration_at_clock_skew_boundary() {
    const BOUNDARY_NOW_UNIX: u64 = 1_700_000_030;
    ...
    let case = suite["cases"]
        ...
        .expect("cases array")
        .iter()
        .find(|case| field(case, "id") == Some("reallyme-jwt/temporal-expired-within-skew"))
        .expect("expiration-within-skew vector");
    let compact = field(case, "compact").expect("compact");
    let public_key = hex_to_bytes(field(case, "public_key_hex").expect("public_key_hex"));
    let jwk: Jwk =
        serde_json::from_value(case["verification_jwk"].clone()).expect("verification_jwk");
    let policy = JwtClaimsValidationPolicy::strict("did:me:verifier");

    let error = decode_verify_jwt_with_claims_validation::<Value>(
        compact,
        &jwk,
        &public_key,
        BOUNDARY_NOW_UNIX,
    ...
    .expect_err("expiration equality must be rejected");
...
fn unsigned_jwt_vectors_decode_or_fail_closed() {
    let suite: Value = serde_json::from_str(include_str!("../../../vectors/unsigned-jwt.json"))
