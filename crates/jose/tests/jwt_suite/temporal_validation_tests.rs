#![allow(deprecated, missing_docs, clippy::expect_used, clippy::unwrap_used)]
...
#[test]
fn strict_temporal_rejects_expiration_at_clock_skew_boundary() {
    let err = temporal_error_for_claims(serde_json::json!({
        "iss": "did:me:test",
        "sub": "alice",
        "exp": NOW_UNIX - 60,
...
#[test]
fn strict_temporal_accepts_expiration_strictly_inside_clock_skew() {
    let claims = serde_json::json!({
        "iss": "did:me:test",
        "sub": "alice",
        "exp": NOW_UNIX - 59,
...
#[test]
fn temporal_rejects_expiration_at_verification_time_without_skew() {
    let policy = JwtTemporalValidationPolicy::new(true, false, false, 0, 0);
    let claims = serde_json::json!({
        "iss": "did:me:test",
        "sub": "alice",
        "exp": NOW_UNIX,
...
#[test]
fn temporal_rejects_skew_above_five_minutes() {
    let claims = serde_json::json!({
        "exp": NOW_UNIX + 300,
    });
    let policy = JwtTemporalValidationPolicy::new(true, false, false, 301, 60);

    let error = verify_claims(&claims, policy).unwrap_err();

    assert!(matches!(error, JwtError::InvalidTemporalPolicy));
}

#[test]
fn temporal_rejects_zero_verification_time_at_native_boundary() {
    let key = gen_ed25519();
    let claims = serde_json::json!({
        "exp": NOW_UNIX + 300,
    });
    ...
    let error = decode_verify_jwt_with_temporal_validation::<serde_json::Value>(
        &compact,
        &key.jwk,
        &key.public,
        0,
    ...
    assert!(matches!(error, JwtError::InvalidVerificationTime));
}
